...
public class QueueUvAnimation : MonoBehaviour
{
  ...
  private int index;
  private int count, allCount =-1;
  private float deltaTime;
  private bool isVisible;
	private bool isFadeHandle = false;
  ...
  private IEnumerator UpdateTiling()
  {
      ...
      if (index >= count)
        index = 0;
      var offset = !isFadeHandle? new Vector2((float) index / ColumnsFadeIn - (index / ColumnsFadeIn), 1 - (index / ColumnsFadeIn) / (float) RowsFadeIn)
