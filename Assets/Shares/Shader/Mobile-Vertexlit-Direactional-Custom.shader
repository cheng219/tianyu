...
//add by 吴江
//添加了_ShadowHard字段,参考其参与的计算,修正了阴影的灰度限制. 本次编写时为了圣龙之谷的创建角色场景服务

Shader "Mobile/VertexLit (Only Directional Lights Custom)" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_ShadowHard ("Shadow Hard", float) = 0
	}
	SubShader {
...
fixed4 frag_surf (v2f_surf IN) : COLOR {
	...
	c.rgb += o.Albedo * min(lm, max(_ShadowHard,atten*2));
	#else
	c.rgb += o.Albedo * lm;
	#endif
	c.a = o.Alpha;
