...
inline float4 AfsSquashNew(in float4 pos, float SquashNew)
{
	...
	float3 projectedVertex = pos.xyz - (dot(planeNormal, pos) + _SquashPlaneNormal.w) * planeNormal + _AfsBillboardCameraForward.xyz * halfPI * pos.y;
...
// Detail bending
//inline float4 afsAnimateVertex(float4 pos, float3 normal, float4 animParams, float4 i_TreeWorldPos)
...
//	Tumbling / Has to be done befor all other deformations
		...
		float twigPhase = frac(i_pivot_fade.x + i_pivot_fade.y + i_pivot_fade.z);
		twigPhase = -vWavesSum.y * ( (0.5f + fBranchPhase) * test);
		twigPhase = -vWavesSum.y * test ;
		float4 vWaves1 = SmoothTriangleWave( _Time.y * float4(fBranchPhase, test + animParams.y * _AfsXtraBending, 1, 1) );
		twigPhase = (vWaves1.x + vWaves1.y) * 0.5;
		float3 windTangent1 = float3(-_Wind.z, _Wind.y, _Wind.x);
		pos.xyz = mul(AfsRotationMatrix(windTangent1, _Wind.w * twigPhase), pos.xyz);
		pos.xyz += branchOrigin;
	#endif
...
//	Preserve Length
	...
	float3 bend = animParams.y * fDetailAmp * vertexnormal * sign(vertexnormal);
		...
		// As bark might be effected too we shoukd simlify it
...
{
	float3 TreeWorldPos = float3(_Object2World[0].w, _Object2World[1].w, _Object2World[2].w);
//	Scale
