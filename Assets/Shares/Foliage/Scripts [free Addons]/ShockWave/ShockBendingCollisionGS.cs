...
public class ShockBendingCollisionGS : MonoBehaviour {
    public static Shader bendingShader = null;
    public static Shader touchingShader = null;
    public static Texture simpleBendingTexture;

    public static Material simpleBendingMaterialInstance = null;
    public static Material touchBendingMaterialInstance = null;
	...
	private AfsShockWave PlayerVars;
	private Vector3 Player_Direction;
	private float Player_Speed;
	
	private int Player1_ID;
	private AfsShockWave PlayerVars1;
    ...
    public TextureNamePair[] texturePairList = new TextureNamePair[2];
    ...
    void Init()
    {
        ...
        if (myRenderer == null)
        {
            Debug.LogError(this.gameObject.name + " 动态草组件获取Renderer引用失败!");
            return;
        }
        if (simpleBendingMaterialInstance == null)
        {
            simpleBendingMaterialInstance = new Material(myRenderer.sharedMaterial);
        }
        if (touchBendingMaterialInstance == null)
        {
            touchBendingMaterialInstance = new Material(myRenderer.sharedMaterial);
        }
        simpleBendingMaterial = simpleBendingMaterialInstance;
        touchBendingMaterial = touchBendingMaterialInstance;




        if (bendingShader == null)
        {
            if (myRenderer.sharedMaterial == null)
            {
                Debug.LogError(this.gameObject.name + " 动态草组件获取shader引用失败!(或为空)");
                return;
            }
            bendingShader = Shader.Find(myRenderer.sharedMaterial.shader.name);
        }

        if (touchingShader == null)
        {
            touchingShader = Shader.Find("AFS/Foliage Shader Touchbending v4");
        }




        simpleBendingMaterial.shader = bendingShader;
        touchBendingMaterial.shader = touchingShader;
