...
public class UITexture : UIBasicSprite
{
    ...
    void OnDestroy()
    {
        if (mTexture != null)
        {
            if (GameCenter.cameraMng != null)
            {
                Camera preview = GameCenter.cameraMng.previewCamera;
                Camera map = GameCenter.cameraMng.mapCamera;
                if (preview != null && preview.targetTexture == mTexture)
                {
                    GameCenter.previewManager.ClearModel();
                    GameCenter.previewManager.ReleasePreviewRT();
                    GameCenter.cameraMng.PreviewCameraActive(false);
                    RenderTexture rt = preview.targetTexture;
                ...
                else if (map != null && map.targetTexture == mTexture)
                {
                    RenderTexture rt = map.targetTexture;
                    map.targetTexture = null;
                    RenderTexture.ReleaseTemporary(rt);
                    rt.Release();
                    rt = null;
                 //   Resources.UnloadUnusedAssets();
